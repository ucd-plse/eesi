...
if [ "$RUN_PIDGIN_OTRNG" = "1" ]; then
	...
	python src/scripts/sort_bugs.py results/$INPUT/pidgin-otrng-bugs.txt > results/$INPUT/pidgin-otrng-bugs-sorted.csv
    fi
fi
...
if [ "$RUN_OPENSSL" = "1" ]; then
    ...
    if [ "$CMD" = "specs" ]; then
        ...
        echo "OpenSSL bugs..."
        (time $EESI_CMD --bitcode $BC --command bugs --specs $OUTPUTSPECS | grep -v WARNING | sort -u) > $OUTPUTBUGS 2> $OUTPUTBUGTIME
	python src/scripts/sort_bugs.py results/$INPUT/openssl-bugs.txt > results/$INPUT/openssl-bugs-sorted.csv
    fi
fi
...
if [ "$RUN_MBEDTLS" = "1" ]; then
    ...
    if [ "$CMD" = "specs" ]; then
        ...
        echo "mbedTLS bugs..."
        (time $EESI_CMD --bitcode $BC --command bugs --specs $OUTPUTSPECS | grep -v WARNING | sort -u) > $OUTPUTBUGS 2> $OUTPUTBUGTIME
	python src/scripts/sort_bugs.py results/$INPUT/mbedtls-bugs.txt > results/$INPUT/mbedtls-bugs-sorted.csv
    fi
fi
...
if [ "$RUN_NETDATA" = "1" ]; then
    ...
    if [ "$CMD" = "specs" ]; then
        ...
        echo "netdata bugs..."
        (time $EESI_CMD --bitcode $BC --command bugs --specs $OUTPUTSPECS | grep -v WARNING | sort -u) > $OUTPUTBUGS 2> $OUTPUTBUGTIME
	python src/scripts/sort_bugs.py results/$INPUT/netdata-bugs.txt > results/$INPUT/netdata-bugs-sorted.csv
    fi
fi
...
if [ "$RUN_LINUX_FS" = "1" ]; then
    ...
    if [ "$CMD" = "specs" ]; then
        ...
        echo "Linux FS bugs..."
        (time $EESI_CMD --bitcode $BC --command bugs --specs $OUTPUTSPECS | grep -v WARNING | sort -u) > $OUTPUTBUGS 2> $OUTPUTBUGTIME
	python src/scripts/sort_bugs.py results/$INPUT/linux-fs-bugs.txt > results/$INPUT/linux-fs-bugs-sorted.csv
    fi
fi
...
if [ "$RUN_LINUX_NFC" = "1" ]; then
    ...
    if [ "$CMD" = "specs" ]; then
        ...
        echo "Linux NFC bugs..."
        (time $EESI_CMD --bitcode $BC --command bugs --specs $OUTPUTSPECS | grep -v WARNING | sort -u) > $OUTPUTBUGS 2> $OUTPUTBUGTIME
	python src/scripts/sort_bugs.py results/$INPUT/linux-nfc-bugs.txt > results/$INPUT/linux-nfc-bugs-sorted.csv
    fi
fi
...
if [ "$RUN_LINUX_FULLKERNEL" = "1" ]; then
    ...
    if [ "$CMD" = "specs" ]; then
        ...
        (time $EESI_CMD --bitcode $BC --command bugs --specs $OUTPUTSPECS | grep -v WARNING | sort -u) > $OUTPUTBUGS 2> $OUTPUTBUGTIME
	python src/scripts/sort_bugs.py results/$INPUT/linux-fullkernel-bugs.txt > results/$INPUT/linux-fullkernel-bugs-sorted.csv
    fi
fi
...
if [ "$RUN_LITTLEFS" = "1" ]; then
    ...
    if [ "$CMD" = "specs" ]; then
        ...
        echo "LittleFS bugs..."
        (time $EESI_CMD --bitcode $BC --command bugs --specs $OUTPUTSPECS | grep -v WARNING | sort -u) > $OUTPUTBUGS 2> $OUTPUTBUGTIME
	python src/scripts/sort_bugs.py results/$INPUT/littlefs-bugs.txt > results/$INPUT/littlefs-bugs-sorted.csv
    fi
fi
...
if [ "$RUN_ZLIB" = "1" ]; then
    ...
    if [ "$CMD" = "specs" ]; then
        ...
        echo "zlib bugs..."
        (time $EESI_CMD --bitcode $BC --command bugs --specs $OUTPUTSPECS | grep -v WARNING | sort -u) > $OUTPUTBUGS 2> $OUTPUTBUGTIME
	python src/scripts/sort_bugs.py results/$INPUT/zlib-bugs.txt > results/$INPUT/zlib-bugs-sorted.csv
    fi
fi
